...
/** Block-state conversion boundary backed by ViaVersion Mappings loaded via ViaNBT. */
public final class ViaMappingsStateMapper {
    private static final Map<Long, Integer> IDS = new ConcurrentHashMap<>();
    private ViaMappingsStateMapper() {}
    ...
    private static int map(WrappedBlockState source, ClientVersion version) {
        // Try ViaVersion Mappings via ViaNBT state mapping table
        try {
